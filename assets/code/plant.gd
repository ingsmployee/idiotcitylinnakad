	...
	if last_stems:
		for stem in last_stems:
			if !stem.ready_to_grow_new:
				continue
			var newborn = base.get_new_stem()
			# if we haven't hit either of the maximum splits and have won the split chance lottery, then split
			if splits_performed < base.max_splits_per_growth && total_splits < base.max_total_splits && (random.randf() < base.chance_to_split):
				splits_performed += 1
				total_splits += 1
				var split_angle = clamp(round(random.randfn(0, 5)), -6, 6)
				if split_angle > 0:
					split_angle = max(3, split_angle)
				else:
					split_angle = min(-3, split_angle)
				print(split_angle)
				var other_newborn = base.get_new_stem()
